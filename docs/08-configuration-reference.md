...
| `indirect` | `boolean` | `true` | Compute the 2-hop indirect impact (`lib/transitive.js`). | **Report-only**, never affects risk or the gate. Set to `false` on a repo where the second hop is too noisy. |
| `gate` | `string` | `'advisory'` | PreToolUse gate mode: `advisory` \| `blocking` \| `off`. | `advisory` (default) warns when no fresh report covers an edited file but **lets it through**; `blocking` refuses the edit (exit 2) until a fresh covering report exists; `off` disables the hook. Read by `hooks/impact-gate.js`. |
