...
|---|---|---|
| **Transitive reachable set** $\mathrm{Reachable}(v)$ | $\lbrace u \in V : v \leadsto u\rbrace $, the set reachable from $v$ over $E$ | There is no $E$. Edges would require resolved call/type references; the tool only has textual name hits, which are neither sound (homonyms) nor complete (reflection, DI). A closure over a wrong edge set is a wrong closure. |
