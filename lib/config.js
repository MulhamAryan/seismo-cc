...
const DEFAULTS = {
  ...
  gitDepth: 400,
  // Compute the 2-hop indirect impact (report-only, never affects risk). Set to
  // false on a repo where the second hop produces too much noise.
  indirect: true,
  // PreToolUse gate mode:
  //   'advisory' (default) — warns when no fresh report covers the edited file,
  //                          but lets the edit through. Removes the friction that
  //                          otherwise makes a team disable the gate.
  //   'blocking'           — refuses the edit (exit 2) until a fresh covering
  //                          report exists. The original strict behavior.
