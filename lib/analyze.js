...
function run(opts = {}) {
  ...
  // 2b. Indirect (2-hop) impact. Report-only: never enters the risk. Bounded and
  // lower-confidence by construction (see lib/transitive.js). Disabled when
  // cfg.indirect === false.
  const directCallerFiles = new Set(allCallers.map(c => c.file));
  const indirect = cfg.indirect === false ? [] : transitive.indirectImpact(root, files, {
    directFiles: directCallerFiles,
    exclude: new Set([...targetFiles, ...symbols.map(s => s.declFile).filter(Boolean), ...directCallerFiles]),
    excludeNames: symbolNames,
  });
