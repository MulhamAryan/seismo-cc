...
function render(data) {
  ...
  // Report-only, lower confidence than direct callers: files that reference the
  // types declared in the direct callers. Never enters the risk.
  if (Array.isArray(data.indirect) && data.indirect.length) {
    p('## Indirect impact (2 hops) — confidence: indirect');
    p('');
    p('Second-order scope: files that reference the types declared in the direct callers above. A change can ripple here without these files ever naming the changed symbol. Lower confidence — two lexical hops, no type resolution — and it does **not** affect the risk level.');
    p('');
    for (const c of data.indirect.slice(0, 20)) {
      p(`- \`${c.file}\` — ${c.count} line(s) · via ${c.via.map(v => `\`${v}\``).join(', ')}`);
    }
    if (data.indirect.length > 20) p(`- … and ${data.indirect.length - 20} more file(s)`);
    p('');
  }
  ...
  // --- Hidden-dependency checks (advisory) ---
  // Lexical scans that turn part of the blind-spot list into "searched and
  // reported". Advisory only: they never affect the risk level or the gate.
  if (Array.isArray(data.hiddenChecks) && data.hiddenChecks.length) {
    p('## Hidden-dependency checks (advisory)');
    p('');
    p('_Advisory / heuristic only — never affects the risk level._ Places the reference search cannot see, found by targeted lexical scans. Each is a *possible* dependency, not a proven one.');
    p('');
    const byKind = {
      'reflection-string': 'Symbol name referenced in string literals — possible reflection / DI / serialization / config',
      'sql-table': 'Entity table name referenced in SQL — possible hardcoded SQL dependency',
      'dynamic-construct': 'Reflection / convention-DI construct near the change',
      'route-concat': 'Route/URL built by concatenation — target not statically resolvable',
    };
    for (const kind of Object.keys(byKind)) {
      const hits = data.hiddenChecks.filter(h => h.kind === kind);
      if (!hits.length) continue;
      p(`**${byKind[kind]}:**`);
      for (const h of hits.slice(0, 15)) {
        const sym = h.symbol ? ` \`${h.symbol}\` —` : '';
        p(`- \`${h.file}\`:${h.line}${sym} \`${String(h.evidence).replace(/\|/g, '\\|').replace(/`/g, "'")}\``);
      }
      p('');
    }
  }
  ...
  p('');
  p('Read this before trusting the report. Some of these are actively searched — see "Hidden-dependency checks" above — but the search is heuristic and the rest stay genuinely invisible:');
  p('');
  p('- Reflection, dynamic activation, `Type.GetType`, convention-based DI containers _(partially checked)_');
  p('- Hardcoded SQL, views and stored procedures, DBMS triggers _(SQL partially checked)_');
  p('- Jobs, business rules and feature flags configured in the database _(needs runtime/DB — not checked)_');
  p('- URLs and handler names built by concatenation _(concatenation flagged, target unresolved)_');
...
function renderShort(data) {
  ...
  if (Array.isArray(data.indirect) && data.indirect.length) L.push(`Indirect impact (2 hops): ${data.indirect.length} file(s)`);
  ...
  if (Array.isArray(data.hiddenChecks) && data.hiddenChecks.length) L.push(`Hidden-dependency checks (advisory): ${data.hiddenChecks.length}`);
