...
function render(data) {
  ...
  if (data.topCallers.length) {
    ...
    for (const c of data.topCallers.slice(0, 25)) {
      ...
      // Marqueur de confiance : on ne l'affiche que si la correspondance n'est pas franche (homonyme probable).
  ...
  // --- Breaking API ---
  // Changements incompatibles de la surface publique : signal fort, on le place juste après la surface publique.
  ...
  // Historique consultatif issu d'seismo-memory : purement informatif, n'entre PAS dans le calcul du risque.
...
function renderShort(data) {
  ...
  // Consultatif seulement : n'influence pas le risque.
